...
class TestInventoryValidation(unittest.TestCase):
    ...
    def test_unsupported_api_version(self):
        inventory = demo.generate_inventory(**demo.ALLINONE)
        inventory['apiVersion'] = 'v0'
        with self.assertRaisesRegex(errors.FailException, schema.ERROR_API_VERSION.format(
                for_procedure='', actual_version='v0', supported_version='.*')):
            demo.new_cluster(inventory)

    def test_unsupported_api_version_procedures(self):
        inventory = demo.generate_inventory(**demo.ALLINONE)
        for procedure in demo._PROCEDURES_WITH_INVENTORY:
            procedure_inventory = demo.generate_procedure_inventory(procedure)
            procedure_inventory['apiVersion'] = 'v0'

            context = demo.create_silent_context(['fake.yaml'], procedure=procedure)

            with self.assertRaisesRegex(errors.FailException, schema.ERROR_API_VERSION.format(
                    for_procedure=f' for procedure {procedure!r}', actual_version='v0', supported_version='.*')):
                demo.new_cluster(deepcopy(inventory), procedure_inventory, context)

    def test_allow_missed_procedure(self):
        inventory = demo.generate_inventory(**demo.ALLINONE)
        for procedure in demo._PROCEDURES_WITH_OPTIONAL_INVENTORY:
