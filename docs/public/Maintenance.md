...
for example if you want to switch back to DNS configuration provided over DHCP. 
In this case you can use `reconfigure` procedure to remove "immutable" attribute from `/etc/resolv.conf` 
and restore original content generated by systemd-resolved or NetworkManager.

To disable KubeMarine management of `/etc/resolv.conf`, run `reconfigure` procedure with following configuration:
```yaml
services:
  resolv.conf: {}
```
...
**Note:**
* If you want to simply change `/etc/resolv.conf` configuration provided by KubeMarine, make changes directly to `cluster.yaml` and use `install` procedure with task `prepare.dns.resolv_conf`.
