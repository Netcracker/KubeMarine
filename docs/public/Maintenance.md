...
#### Disable resolv.conf KubeMarine management

Sometimes you may want to stop Kubemarine from managing `/etc/resolv.conf` on your cluster nodes—for example, if you want to revert to the DNS configuration provided via DHCP. In this case, you can use the `reconfigure` procedure to remove the immutable attribute from `/etc/resolv.conf` and restore the original content generated by **systemd‑resolved** or **NetworkManager**.

To disable Kubemarine management of `/etc/resolv.conf`, run the `reconfigure` procedure with the following configuration:
```yaml
services:
  resolv.conf: {}
```
...
**Note:**
* If you simply want to modify the `/etc/resolv.conf` configuration that Kubemarine provides, edit `cluster.yaml` directly and invoke the `install` procedure with the task `prepare.dns.resolv_conf`.
