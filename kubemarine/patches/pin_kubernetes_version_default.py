...
PATCH_ID = "pin_kubernetes_version"
...
class TheAction(Action):
    ...
    def run(self, res: DynamicResources) -> None:
        ...
        if 'kubernetesVersion' not in inventory.get('services', {}).get('kubeadm', {}):
            logger.debug("Set services.kubeadm.kubernetesVersion = %s in the inventory",
                         PREVIOUS_DEFAULT_KUBERNETES_VERSION)
            inventory.setdefault('services', {}).setdefault('kubeadm', {})['kubernetesVersion'] = \
                PREVIOUS_DEFAULT_KUBERNETES_VERSION
        else:
