...
def install(group: NodeGroup) -> RunnersGroupResult:
    ...
    with group.new_executor() as exe:
        ...
        for node in exe.group.get_ordered_members_list():
            node.sudo('rm -rf /etc/systemd/system/kubelet*', dry_run=dry_run)
            node.get_node_name()
            ...
            node.sudo("chmod 644 /etc/systemd/system/kubelet.service", dry_run=dry_run)

        log.debug("\nReloading systemd daemon...")
        system.reload_systemctl(exe.group)
        exe.group.sudo('systemctl enable kubelet', dry_run=dry_run)
...
def join_other_control_planes(group: NodeGroup) -> RunnersGroupResult:
    ...
    return first_control_plane.sudo("kubectl get pods --all-namespaces -o=wide", dry_run=dry_run)
...
def get_join_dict(group: NodeGroup, dry_run=False) -> dict:
    join_dict = {}
    if dry_run:
...
def init_first_control_plane(group: NodeGroup) -> None:
    ...
    first_control_plane.sudo("mkdir -p /etc/kubernetes", dry_run=dry_run)
    first_control_plane.put(io.StringIO(config), '/etc/kubernetes/init-config.yaml', sudo=True, dry_run=dry_run)
...
def init_workers(group: NodeGroup) -> None:
    cluster: KubernetesCluster = group.cluster
    dry_run = utils.check_dry_run_status_active(cluster)
    join_dict = cluster.context.get("join_dict", get_join_dict(group, dry_run))
    join_config = {
...
def upgrade_first_control_plane(upgrade_group: NodeGroup, cluster: KubernetesCluster, dry_run=False, **drain_kwargs):
    ...
    first_control_plane.sudo(drain_cmd, hide=False, dry_run=dry_run)

    upgrade_cri_if_required(first_control_plane, dry_run=dry_run)
    ...
    first_control_plane.sudo(
        ...
        f"sudo systemctl restart kubelet", hide=False, dry_run=dry_run)
    if dry_run:
...
def upgrade_other_control_planes(upgrade_group: NodeGroup, cluster: KubernetesCluster, dry_run=False, **drain_kwargs):
    ...
    for node in cluster.nodes['control-plane'].get_ordered_members_list():
        ...
        if node_name != first_control_plane.get_node_name():
            ...
            drain_cmd = prepare_drain_command(cluster, node_name, **drain_kwargs)
            node.sudo(drain_cmd, hide=False, dry_run=dry_run)

            upgrade_cri_if_required(node, dry_run=dry_run)
...
def patch_kubeadm_configmap(first_control_plane: NodeGroup, cluster: KubernetesCluster, dry_run=False):
    '''
    ...
    first_control_plane.put(io.StringIO(result_config), "/tmp/kubeadm_config.yaml", sudo=True, dry_run=dry_run)
...
def upgrade_workers(upgrade_group: NodeGroup, cluster: KubernetesCluster, dry_run=False, **drain_kwargs):
    ...
    for node in cluster.nodes['worker'].exclude_group(cluster.nodes['control-plane'])\
        ...
        first_control_plane.sudo("kubectl uncordon %s" % node_name, hide=False, dry_run=dry_run)
        if dry_run:
            cluster.log.debug("[dry-run]Nodes have correct Kubernetes version...")
            cluster.log.debug("[dry-run] Pods are ready!")
            continue
...
def create_kubeadm_patches_for_node(cluster: KubernetesCluster, node: NodeGroup, dry_run=False):
...
def fix_flag_kubelet(cluster: KubernetesCluster, node: NodeGroup, dry_run=False):
