...
admission_template = "../templates/admission.conf.j2"
...
#        .render(nodes=future_nodes, bindings=bindings,config_options=cluster.inventory['rbac']['pss'])
#####################################
...
def enrich_inventory(inventory, _):
    ...
    # if security enabled, then add PodSecurity feature gates 
...
def verify_config(inventory):
    switch = inventory["rbac"]["admission"]
    flag = inventory["rbac"]["pss"]["pod-security"]
    profile = inventory["rbac"]["pss"]["profile"]
    exemptions = inventory["rbac"]["pss"]["exemptions"]
    if switch not in valid_switches:
        raise Exception("incorrect value for 'admission', valid values: %s" % valid_switches)
    if flag not in valid_flags:
        raise Exception("incorrect value for 'pod-security', valid values: %s" % valid_flags)
    if profile not in valid_flags:
        raise Exception("incorrect value for 'profile', valid values: %s" % valid_profile)
    if exemptions:
        for exemption in exemptions:
            if exemption not in valid_exemptions:
                raise Exception("incorrect value for 'exemption', valid values: %s" % valid_exemptions)
...
def install(cluster):
    ...
    first_master.call(manage_policies,
                      manage_type="apply",
                      ###########################################################################################
                      manage_scope=resolve_oob_scope(cluster.inventory["rbac"]["pss"], "enabled"))
                      ###########################################################################################

    cluster.log.debug("Labeling namespaces...")
    first_master.call(manage_policies,
                      manage_type="apply",
...
def update_kubeadm_configmap(first_master, target_state):
    ...
    # update kubeadm config map with enabled feature
    cluster_config["apiServer"]["extraArgs"]["feature-gates"] = final_plugins_string
