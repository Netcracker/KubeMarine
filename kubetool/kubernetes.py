...
from kubetool import system, plugins, psp, etcd, cri, packages
...
def patch_kubeadm_configmap(first_master, cluster):
    ...
    cluster_configuration_yaml_audit = config_map["data"]["ClusterConfiguration"]["apiServer"]["extraArgs"]["audit-log-path"]
    ...
    if not cluster_configuration_audit.get("audit-log-path", {}):
        cluster_configuration_audit["audit-log-path"] = {}
        cluster_configuration_audit['apiServer']['extraArgs']['audit-log-path'] = ("%s" % cluster_configuration_audit["audit-log-path"])
...
def upgrade_workers(version, upgrade_group, cluster, drain_timeout=None, grace_period=None):
    first_master = cluster.nodes['master'].get_first_member(provide_node_configs=True)
    for node in cluster.nodes['worker'].exclude_group(cluster.nodes['master']).get_ordered_members_list(
