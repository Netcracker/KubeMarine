...
# running any kubetools code (and any threads it may create) ensures that
...
# In Kubetool stdout messages writes only to stdout - no stderr messaging at all,
...
def main():
    ...
    if len(arguments) > 0:
        if arguments[0] == 'selftest':
            ...
            print('Kubetools %s' % release_version)
            return
...
def selftest():
    ...
    for procedure, procedure_details in procedures.items():
        ...
        del module
        del sys.modules['kubetool.procedures.%s' % procedure]
