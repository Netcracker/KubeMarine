...
from kubetool import packages, system
from kubetool.core import utils, flow
...
def export_nodes(cluster):
    ...
    backup_command = 'cd /tmp && ' \
                     'sudo tar -czvf /tmp/kubetools-backup.tar.gz -P $(sudo readlink -e %s) && ' \
                     'sudo ls -la /tmp/kubetools-backup.tar.gz && ' \
                     'sudo du -hs /tmp/kubetools-backup.tar.gz' % (' '.join(backup_list))
    ...
    for node in cluster.nodes['all'].get_ordered_members_list(provide_node_configs=True):
        node['connection'].get('/tmp/kubetools-backup.tar.gz',
    ...
    cluster.nodes['all'].sudo('rm -f /tmp/kubetools-backup.tar.gz')
